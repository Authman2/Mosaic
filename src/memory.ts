...
import { isViewFunction, isBooleanAttribute, nodeMarker, isMosaicDefined, objectFromArray, runLifecycle, insertAfter } from "./util";
...
export default class Memory {
    ...
    commitNode(el: Element, oldv: any, newv: any) {
        ...
        if(typeof newv === 'object' && newv.__isKeyedArray) {
            console.log('Found array', el, newv);
            this.commitArray(el, oldv, newv);
        }
        ...
        else if(newv instanceof DocumentFragment) {
            el.replaceWith(newv.cloneNode(true));
        }
        else {
            el.replaceWith(newv);
        }
    ...
    * multiple attributes. */
    ...
    commitArray(el: HTMLElement|ChildNode|ShadowRoot, oldv: any, newv: any) {
        const oldItems = oldv && typeof oldv === 'object' && oldv.__isKeyedArray 
            ? oldv.items : [];
        const newItems = newv && typeof newv === 'object' && newv.__isKeyedArray 
            ? newv.items : [];
        ...
        for(let i = 0; i < diffs.length; i++) {
            ...
            // Handle "add" operations.
                ...
                let ref: HTMLElement|ChildNode|null|ShadowRoot = el;
